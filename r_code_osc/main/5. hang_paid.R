hehe_cargo <- reactiveVal(NULL)  # ✅ nơi lưu dữ liệu kết quả
check_cargo <- reactiveVal(NULL) 
folder_cargo <- "www/cargo/paid"
observeEvent(input$run_cargo, {
  req(input$file_cargo,input$file_cargo_tn)
  ...
  tryCatch({
    withProgress(message = "Đang xử lý dữ liệu...", {
      ...
      files <- input$file_cargo$datapath
      names(files) <- input$file_cargo$name
      ...
      file_tn <- input$file_cargo_tn$datapath
      file_path <- file_tn[1]
      sheets1 <- excel_sheets(file_path)
      sheets_paid <- sheets1[str_detect(sheets1, regex("paid", ignore_case = TRUE))]
      if (length(sheets_paid) == 0) {
      ...
      df_paid = df_paid %>%
        ...
        select(-vnd, -usd)
      pos_paid <- grep("^usd", colnames(df_paid), ignore.case = TRUE)
    
      df_paid$paid_osc <-
        coalesce(as.numeric(gsub("[^0-9.-]", "", df_paid[[pos_paid[1]]])), 0) * usd_rate +
        coalesce(as.numeric(gsub("[^0-9.-]", "", df_paid[[pos_paid[1] - 1]])), 0)
      
      
  
      #___________________________ VNĐ
      vnd <- data_list$vnd
      pos_vnd <- grep("quy_doi", colnames(vnd), ignore.case = TRUE)
      if (length(pos_vnd) == 0) {
        stop("❌ Không tìm thấy cột 'quy_doi' trong sheet USD")
      }
      vnd$paid_osc <- coalesce(as.numeric(gsub("[^0-9.-]", "", vnd[[pos_vnd[1]]])), 0) 
      
      #___________________________ USD
      usd <- data_list$usd
      pos_usd <- grep("quy_doi", colnames(usd), ignore.case = TRUE)
      if (length(pos_usd) == 0) {
        stop("❌ Không tìm thấy cột 'quy_doi' trong sheet USD")
      }
      ...
      pos_ij <- grep("^usd", colnames(ngoai_ij), ignore.case = TRUE)
      
      ngoai_ij$paid_osc <-
        coalesce(as.numeric(gsub("[^0-9.-]", "", ngoai_ij[[pos_ij[1]]])), 0) * usd_rate +
        coalesce(as.numeric(gsub("[^0-9.-]", "", ngoai_ij[[pos_ij[1] - 1]])), 0)
      ...
      hehe_cargo(hehe)  # ✅ Lưu vào reactiveVal để dùng cho download
      # ==============================
      # 2. SHOW FREQ MODAL
      # ==============================
      ...
      showModal(
        modalDialog(
          ...
          easyClose = FALSE,
          footer = tagList(
            actionButton("freq_cargo", "Tôi đã hiểu", class = "btn-primary")
          )
        )
      )
    })
...
observeEvent(input$freq_cargo, {
  ...
  tryCatch({
    ...
    file_name_rds <- file.path(folder_cargo, paste0( input$year, "_",tolower(input$quarter) ,  ".rds"))
    print(file_name_rds)
...
observeEvent(input$overwrite, {
  ...
  tryCatch({
    file_name_rds <- file.path(folder_cargo, paste0( input$year, "_",tolower(input$quarter) ,  ".rds"))
    saveRDS(hehe_cargo(), file_name_rds)
    showNotification(paste("✅ Đã lưu file:", basename(file_name_rds)), type = "message")
    hehe = hehe_cargo()
    # ✅ 2️⃣ Đường dẫn file RDS
    paid_file <- "www/paid_cargo.rds"
...
# ✅ Nút download: truy cập dữ liệu qua hehe_cargo()
output$download_cargo <- downloadHandler(
  
  filename = function() {
    paste0("cargo_paid_",  input$year, "_",tolower(input$quarter) , ".xlsx")
  },
  content = function(file) {
    req(hehe_cargo())  # đảm bảo có dữ liệu
    writexl::write_xlsx(hehe_cargo(), file)
  }
)


# ✅ Nút download: truy cập dữ liệu qua hehe_cargo()
output$download_cargo_check <- downloadHandler(
  filename = function() {
    paste0("cargo_check_paid_",input$year, "_",tolower(input$quarter) , ".xlsx")
  },
  content = function(file) {
    req(check_cargo())  # đảm bảo có dữ liệu
    writexl::write_xlsx(check_cargo(), file)
  }
)
