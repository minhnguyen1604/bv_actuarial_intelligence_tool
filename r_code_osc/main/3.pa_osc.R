hehe_pa1 <- reactiveVal(NULL)  # ✅ nơi lưu dữ liệu kết quả
check_pa1 <- reactiveVal(NULL) 
folder_pa1 <- "www/pa/osc"
observeEvent(input$run_pa1, {
  req(input$file_pa1)
  ...
  tryCatch({
    withProgress(message = "Đang xử lý dữ liệu...", {
      ...
      files <- input$file_pa1$datapath
      names(files) <- input$file_pa1$name
      ...
      col_names <- c(
        ...
        "SoSDBS_MaSDBS", "SoSDBS_MaCty", "SoSDBS_MaPhong", "SoSDBS_MaNvụ", "SoSDBS_MaNam", "SoSDBS_MaHopDong",
        "SoTrenIJ",
        "NguoiDuocBaoHiem",
      ...
      hehe <- hehe %>%
        mutate(check_missing_dates = if_any(
          c(NgayTonThat, ThoiHanBaoHiem_Tu, ThoiHanBaoHiem_Den, NgayThongBao ),
          is.na
        ))
      # ==============================
      # 2. SHOW FREQ MODAL
      ...
      hehe_pa1(hehe)  # ✅ Lưu vào reactiveVal để dùng cho download
      ...
      showModal(
        modalDialog(
          ...
          easyClose = FALSE,
          footer = tagList(
            actionButton("freq_pa1", "Tôi đã hiểu", class = "btn-primary")
          )
        )
      )
    })
...
observeEvent(input$freq_pa1, {
  ...
  tryCatch({
    hehe <- hehe_pa1()
      file_name_rds <- file.path(folder_pa1, paste0( input$year, "_",tolower(input$quarter) , ".rds"))
      print(file_name_rds)
...
observeEvent(input$overwrite, {
  ...
  tryCatch({
    file_name_rds <- file.path(folder_pa1, paste0( input$year, "_", tolower(input$quarter),  ".rds"))
    saveRDS(hehe_pa1(), file_name_rds)
    showNotification(paste("✅ Đã lưu file:", basename(file_name_rds)), type = "message")
    hehe = hehe_pa1()
    # ✅ 2️⃣ Đường dẫn file RDS
    paid_file <- "www/paid_pa.rds"
...
# ✅ Nút download: truy cập dữ liệu qua hehe_pa1()
output$download_pa1 <- downloadHandler(
  
  filename = function() {
    paste0("pa_osc_",  input$year, "_", tolower(input$quarter), ".xlsx")
  },
  content = function(file) {
    req(hehe_pa1())  # đảm bảo có dữ liệu
    writexl::write_xlsx(hehe_pa1(), file)
  }
)


# ✅ Nút download: truy cập dữ liệu qua hehe_pa1()
output$download_pa_check1 <- downloadHandler(
  filename = function() {
    paste0("pa_check_osc_",input$year, "_", tolower(input$quarter), ".xlsx")
  },
  content = function(file) {
    req(check_pa1())  # đảm bảo có dữ liệu
    writexl::write_xlsx(check_pa1(), file)
  }
)
