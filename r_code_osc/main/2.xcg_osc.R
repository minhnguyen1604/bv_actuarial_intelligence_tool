hehe_xcg1 <- reactiveVal(NULL)  # ✅ nơi lưu dữ liệu kết quả
check_xcg1 <- reactiveVal(NULL) 
folder_xcg1 <- "www/xcg/osc"

observeEvent(input$run_xcg1, {
  req(input$file_xcg1)
  ...
  tryCatch({
    withProgress(message = "Đang xử lý dữ liệu...", {
      ...
      files <- input$file_xcg1$datapath
      names(files) <- input$file_xcg1$name
      # ✅ Đọc từng file, chỉ lấy sheet có >10 cột
      df_list <- map(files, function(file_path) {
        sheets <- excel_sheets(file_path)
        
        # Tìm sheet hợp lệ (số cột > 10)
        valid_sheet <- NULL
        for (s in sheets) {
          tmp <- suppressMessages(read_excel(file_path, sheet = s, n_max = 5))
          if (ncol(tmp) > 10) {
            valid_sheet <- s
            break
          }
        ...
        # Nếu không có sheet hợp lệ → bỏ qua file
        if (is.null(valid_sheet)) {
          showNotification(
            paste("⚠️ Bỏ qua file", basename(file_path), "- không có sheet nào >10 cột"),
            type = "warning"
          )
          return(NULL)
        }
      ...
      # Gộp các file hợp lệ
      df <- df_list %>%
        discard(is.null) %>%        # bỏ các file không hợp lệ
      ...
      new_names[grepl("CLAIM_URN|so kn|so.*khieu.*nai", new_names, ignore.case = TRUE)] <- "so_kn"
      new_names[grepl("HOLDER|KHaCH|^Chu", new_names, ignore.case = TRUE)] <- "chu_xe"
      new_names[grepl("BKS|bien", new_names, ignore.case = TRUE)] <- "bks"
      new_names[grepl("POLICY_URN|ĐƠN|don", new_names, ignore.case = TRUE)] <- "don_ij"
      new_names[grepl("TO$|THuC", new_names, ignore.case = TRUE)] <- "ngay_ket_thuc"
      new_names[grepl("FROM$|LuC", new_names, ignore.case = TRUE)] <- "ngay_hieu_luc"
      
      new_names[grepl("LOSS|THaT", new_names, ignore.case = TRUE)] <- "ngay_ton_that"
      new_names[grepl("REPORTED|BaO", new_names, ignore.case = TRUE)] <- "ngay_thong_bao"
      new_names[grepl("boi|Paid|du_phong|osc|chua_bt|chua.*boi.*thuong", new_names, ignore.case = TRUE)] <- "paid_osc"
      new_names[grepl("BUSINESS|KD|phong", new_names, ignore.case = TRUE)] <- "phong_kd"
      new_names[grepl("duyet", new_names, ignore.case = TRUE)] <- "ngay_duyet_boi_thuong"
      
      new_names[grepl("n.*bt", new_names, ignore.case = TRUE)] <- "ngay_boi_thuong"
      
      new_names[grepl("thu ly", new_names, ignore.case = TRUE)] <- "thu_ly"
      new_names[grepl("TT_KN|trang_thai", new_names, ignore.case = TRUE)] <- "tt_kn"
      ...
      df <- df %>%
        mutate(
          cong_ty_clean = cong_ty %>%
            ...
            str_remove_all("công ty bảo việt|bảo việt|bv|_osc") %>%
            str_squish() %>%
            str_to_title() %>%
        ...
        mutate(
          cong_ty_clean = cong_ty_clean %>%
            ...
            str_replace_all("^Hue", "Thua Thien Hue")
        )
      ...
      hehe_xcg1(hehe)  # ✅ Lưu vào reactiveVal để dùng cho download
      # ==============================
      # 2. SHOW FREQ MODAL
      # ==============================
      # --- Tính freq ---
      ...
      showModal(
        modalDialog(
          ...
          easyClose = FALSE,
          footer = tagList(
            actionButton("freq_xcg1", "Tôi đã hiểu", class = "btn-primary")
          )
        )
      )
    })
...
observeEvent(input$freq_xcg1, {
  ...
  tryCatch({
    hehe <- hehe_xcg1()
      ...
      file_name_rds <- file.path(folder_xcg1, paste0( input$year, "_", tolower(input$quarter),  ".rds"))
      print(file_name_rds)
      if (file.exists(file_name_rds)) {
        ...
        #check_xcg1( hehe %>% group_by(bks, ngay_ton_that, pham_vi_bt) %>% filter(n()>1)  )
...
observeEvent(input$overwrite, {
  ...
  tryCatch({
    file_name_rds <- file.path(folder_xcg1, paste0( input$year, "_", tolower(input$quarter),  ".rds"))
    saveRDS(hehe_xcg1(), file_name_rds)
    showNotification(paste("✅ Đã lưu file:", basename(file_name_rds)), type = "message")
    hehe = hehe_xcg1()
    # ✅ 2️⃣ Đường dẫn file RDS
    paid_file <- "www/paid_xcg.rds"
...
# ✅ Nút download: truy cập dữ liệu qua hehe_xcg1()
output$download_xcg1 <- downloadHandler(
  
  filename = function() {
    paste0("xcg_osc_",  input$year, "_", tolower(input$quarter), ".xlsx")
  },
  content = function(file) {
    req(hehe_xcg1())  # đảm bảo có dữ liệu
    writexl::write_xlsx(hehe_xcg1(), file)
  }
)


# ✅ Nút download: truy cập dữ liệu qua hehe_xcg1()
output$download_xcg_check1 <- downloadHandler(
  filename = function() {
    paste0("xcg_check_osc_",input$year, "_", tolower(input$quarter), ".xlsx")
  },
  content = function(file) {
    req(check_xcg1())  # đảm bảo có dữ liệu
    writexl::write_xlsx(check_xcg1(), file)
  }
)
